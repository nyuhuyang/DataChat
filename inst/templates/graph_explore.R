...
if (is.null(edges_df) || is.null(nodes_df)) {
  ...
  logs <- c(logs, sprintf("[graph_explore] node_id=%s name=%s node_key=%s node_type=%s edge_type=%s",
  ...
  if (length(params) == 0) {
    ...
    if (nrow(edges_work) > 0) {
      ...
      logs <- c(logs, sprintf("[graph_explore] Nodes kept after edge alignment: %d", nrow(nodes_work)))
    }
